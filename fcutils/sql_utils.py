...
def joinList(arr, prefix = '`', suffix = '`', delimiter=' , '):
    ...
    """
    return delimiter.join(prefix + str(a) + suffix for a in arr)
...
def fieldStrFromList(a, b, vPrefix = '"', vSuffix = '"', delimiter=' , '):
    ...
    """
    c = list(zip(a, b))
    arr = [' `' + str(k) + '`=' + vPrefix + str(v) + vSuffix + ' ' for k, v in c if v != None]
...
def fieldStr(d, vPrefix = '"', vSuffix = '"', delimiter=' , '):
    ...
    --
        ...
        return: `a`="%1%"  AND  `b`="%2%"  AND  `c`="%3%"
    """
    arr = [' `' + str(k) + '`=' + vPrefix + str(v) + vSuffix + ' ' for k, v in d.items() if v != None]
    return joinList(arr, prefix = '', suffix = '', delimiter = delimiter)
...
def fieldSplit(d):
    ...
    --
        @example 
        	fieldStr2({'name':'张三', 'age':None, 'aaa':'bbb'})
